...
<div class="py-10 max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
    ...
    <div class="mt-10 sm:mt-0">
        <div class="md:grid md:grid-cols-3 md:gap-6">
            <div class="md:col-span-1 flex justify-between">
                <div class="px-4 sm:px-0">
                    <h3 class="text-lg font-medium text-gray-900">{{__('Create new admin user')}}</h3>
                    <p class="mt-1 text-sm text-gray-600">
                        {{--__('Here is where you can change your password.')--}}
                    </p>
                </div>
            ...
            <div class="mt-5 md:mt-0 md:col-span-2 py-6">
                <form method="POST" action="{{ route('create_new_admin') }}">
                    ...
                    <div class="flex items-center justify-end px-4 py-3 bg-gray-50 text-right sm:px-6 shadow sm:rounded-bl-md sm:rounded-br-md">
                        @if(session()->has('create_admin_message'))
                            <div class="text-md text-green-600 mr-3 font-bold">
                                {{ session('create_admin_message')}}
                            </div>
                        @endif
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition">
                        Save
